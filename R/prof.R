...
fgData <- function(stacks, counts, reorder = c("alpha", "hot", "time"),
                   colormap) {
    mx <- max(sapply(stacks, length))
...
flameGraph <- function(pd, svgfile, order = c("hot", "alpha", "time"),
    ...
    if (! missing(svgfile))
        svgFlameGraph(svgfile, stacks, counts, order, colormap, main, tooltip)
    else
...
srcSummary <- function(pd, byTotal = TRUE,
    ...
    ## order rows by row number within file and alphabetically by file.
    ord <- order(line)
    val <- val[ord,]
    file <- file[ord]
    val <- val[order(file),]
