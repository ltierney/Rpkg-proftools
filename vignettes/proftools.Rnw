...
\citep{Gentry:Rgraphviz} packages from Bioconductor to render an
...
\section{Fixups}
This function does more or less what \code{hotPaths} trimming does for
\code{total.pct = 100 p} but not as efficiently. Might be useful to
add to \code{filterProfileData}, maybe as a \code{merge.pct} filter:
<<>>=
...
environment(mergeStacks) <- environment(hotPaths)
@ %%
This does a nice job; should do about what hot paths trimming on total does.
<<fig.cap="Better filtered view">>=
plotProfileCallGraph(mergeStacks(pd, .05))
@ %%

% The \code{focudPD} code seems more reasonable than this.
...
environment(dropSourceFrames) <- environment(hotPaths)
@ 
