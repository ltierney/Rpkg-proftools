...
@
\\
The plot doesn't look very readable, but focusing on a particular
section will make it so, and we show how to do this later in this
paper.
\newpage
We can also plot a hot paths flamegraph, time graph, or alphabetical
flame graph by using "hot", "time", or "alpha" in the order argument
below.
...
pd object. We can filter the pd object by selecting or dropping
