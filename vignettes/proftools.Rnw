...
foo <- function(pd, p){
    ...
    repeat {
        ...
        if (sum(low) == 0 || max(lns[low]) <=1)
            return(pd)
        idx <- which(low & lns == max(lns[low]))
        for (i in idx)
            pd$stacks[[i]] <- trim(pd$stacks[[i]])
        if (pd$haveRefs)
            for (i in idx)
                pd$refs[[i]] <- trim(pd$refs[[i]])
        pd <- proftools:::compactPD(pd)
    }
}
@ %%
...
New version of \code{cvtProfileData}:
<<>>=
cvtProfileData <- function(pd, GC) {
    if (inherits(pd, "proftools_profData")) {
        ...
        v <- eapply(data, function(x) x$total)
        drop <- names(v)[v < pd$total * 0.05]
        rm(list = drop, envir = data)
        eapply(data, function(x) {
            e <- x$edges
            rm(list = intersect(names(e), drop), envir = e)
        })
...
@ %
Apparently something in \code{getProfCallGraphNodeEntry} is looking
into \code{pd} for names instead of getting them from \code{data}.
