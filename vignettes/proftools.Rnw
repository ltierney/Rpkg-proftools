...
use the \code{focus} argument to also remove all calls preceding the first
\code{glm} call from the selected stack frames:
<<>>=
...
produced by \code{gprof}, can be obtained with \code{printProfileCallGraph}.
For the subgraph of the \code{glm.fit} calls, for example:
<<>>=
