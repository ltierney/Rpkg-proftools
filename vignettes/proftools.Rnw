...
%% **** simplify using knitr chunk opitons
...
with lower hit total counts are dropped. The \code{maxnodes} argument
...
\code{"alpha"} ordering orders the calls alphabetically.  The ‘"time"’
ordering preserves the original time ordering within the stack trace
file.
...
\begin{figure}
  ...
  \end{knitrout}
  \caption{\code{kcachegrind} interface for examining profile profile
