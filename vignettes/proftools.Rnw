...
visualizing, and subsetting profiling data.
...
<<>>=
filteredPD <- filterProfileData(pd, select = "withVisible")
filteredPD <- filterProfileData(filteredPD, skip = 4)
@ %%
...
We can further narrow our examination to the calls to \code{glm}, and
use the \code{focus} argument to remove all calls preceding the first
glm call from the selected stack frames:
<<>>=
glmPD <- filterProfileData(filteredPD, select = "glm", focus = TRUE)
hotPaths(glmPD, maxdepth = 3)
@ %%

The details of the \code{filterProfileData} interface may change
before release to make it easier to express a pipeline of
transformations within one call.
...
\pkg{graph} and \pkg{Rgraphviz} from Bioconductor to render an
...
@ 
and shown in Figure \ref{fullCallGraphFig}
\begin{figure}
  \centering
<<fig=true,echo=false>>=
<<fullCallGraphCmd>>
...
<<bootCallGraphCmd>>=
plotProfileCallGraph(filterProfileData(pd, select = "boot",
                                       focus = TRUE,
                                       total.pct = 5,
                                       maxdepth = 10))
@ 
...
produced by \code{gprof}, can be obtained with
<<eval=FALSE>>=
...
package. The development version provides two interfaces, one based on
\pkg{gWidgets2} and one on \pkg{shiny}. At the time of writing these
interfaces are started with
<<eval=false>>=
startWidget(pd)
runShiny(pd)
@ %%
but this will certainly change. Also the shiny interface currently
gets into an infinite recursion for this example; we will sort that
out. The current interfaces are on the \code{graphics} branch in the
Git repository for \pkg{proftoolsGUI}; they will be merged into the
\code{master} branch shortly.
\end{document}
