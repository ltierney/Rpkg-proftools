...
style used in Google's profiling library but can be customized in a
number of ways.  A call graph for the full profile data is produced by
<<fullCallGraphCmd>>=
plotProfileCallGraph(pd)
@ 
and shown in Figure \ref{fullCallGraph}
\begin{figure}
  \centering
<<fig=true,echo=false>>=
<<fullCallGraphCmd>>
...
\label{bootCallGraph}
\end{figure}
The result is shown in Figure \ref{bootCallGraph}.
...
Another visualization sometimes used is a flame graph. This is
essentially a visual representation of the hot path summary.
<<fig=true>>=
...
@ %%
The \code{boot} calls precede the data generation and the \code{lm} call.
